...
class WebsiteCategoryController extends Controller
{
    ...
    public function left(Request $request)
    {
        ...
        $informations = Information::limit(10)->where('status', true)->orderBy('order')->get();
        ...
        $area_lists['country']['websites'] = Website::where([['area_id', 1], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
        ...
        switch ($area_type)
        {
            case '省':
                ...
                $area_lists['province']['websites'] = Website::where([['area_id', $area_id], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                break;
            case '市':
                $city = Area::find($area_id);
                $area_lists['province']['province_name'] = Area::whereId($city->parent_id)->value('name');
                $area_lists['province']['websites'] = Website::where([['area_id', $city->parent_id], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                $area_lists['city']['city_name'] = Area::whereId($city->id)->value('name');
                $area_lists['city']['websites'] = Website::where([['area_id', $city->id], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                $area_lists['district']['district_name'] = Area::whereId(($city->id)+1)->value('name');
                $area_lists['district']['websites'] = Website::where([['area_id', ($city->id)+1], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                break;
            case '区':
                ...
                $area_lists['city']['websites'] = Website::where([['area_id', $district->parent_id], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                $area_lists['district']['district_name'] = Area::whereId($district->id)->value('name');
                $area_lists['district']['websites'] = Website::where([['area_id', $district->id], ['website_category_id', 16], ['status', true]])->orderBy('order')->get();
                break;
        }
    ...
    public function lists(Request $request)
    {
        ...
        $area_lists = [];
        $area_lists['country']['country_name'] = Area::whereId(1)->value('name');
        $area_lists['country']['websites'] = Website::where([['area_id', 1], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();

        switch ($area_type)
        {
            case '省':
                $area_lists['province']['province_name'] = Area::whereId($area_id)->value('name');
                $area_lists['province']['websites'] = Website::where([['area_id', $area_id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                break;
            case '市':
                $city = Area::find($area_id);
                $area_lists['province']['province_name'] = Area::whereId($city->parent_id)->value('name');
                $area_lists['province']['websites'] = Website::where([['area_id', $city->parent_id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                $area_lists['city']['city_name'] = Area::whereId($city->id)->value('name');
                $area_lists['city']['websites'] = Website::where([['area_id', $city->id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                $area_lists['district']['district_name'] = Area::whereId(($city->id)+1)->value('name');
                $area_lists['district']['websites'] = Website::where([['area_id', ($city->id)+1], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                break;
            case '区':
                $district = Area::find($area_id);
                $city = Area::find($district->parent_id);
                ...
                $area_lists['province']['websites'] = Website::where([['area_id', $city->parent_id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                $area_lists['city']['city_name'] = Area::whereId($district->parent_id)->value('name');
                $area_lists['city']['websites'] = Website::where([['area_id', $district->parent_id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                $area_lists['district']['district_name'] = Area::whereId($district->id)->value('name');
                $area_lists['district']['websites'] = Website::where([['area_id', $district->id], ['website_category_id', 1], ['status', true]])->orderBy('order')->get();
                break;
        }
